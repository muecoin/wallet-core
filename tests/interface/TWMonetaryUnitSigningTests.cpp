...
TEST(MonetaryUnitSigning, SignP2WPKH) {
    ...
    auto result = TW::Bitcoin::TransactionSigner<MonetaryUnit::Transaction>(std::move(input)).sign();
...
TEST(MonetaryUnitSigning, SignP2PKH) {
    Proto::SigningInput input;
    input.set_hash_type(TWSignatureHashTypeAll);
    input.set_amount(2500);
    input.set_byte_fee(1);
...
TEST(MonetaryUnitSigning, SignP2SH_P2WPKH) {
    // TX outputs
    Proto::SigningInput input;
    input.set_hash_type(TWSignatureHashTypeAll);
